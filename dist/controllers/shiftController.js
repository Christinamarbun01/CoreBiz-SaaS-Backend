...
export const openShift = async (req, res) => {
    try {
        ...
        if (insertError)
            throw insertError;
        if (!shift)
            throw new Error('Gagal membuat shift (no data returned)');
...
export const closeShift = async (req, res) => {
    try {
        ...
        if (cashError)
            throw cashError;
        const total_cash_payments = (cashPayments || []).reduce((sum, p) => sum + (p.amount || 0), 0);
        ...
        if (updateError)
            throw updateError;
        if (!updatedShift)
            throw new Error('Gagal memperbarui data shift');
        return res.status(200).json({
