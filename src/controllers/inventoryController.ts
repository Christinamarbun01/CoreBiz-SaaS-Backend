...
// ---------------------------------------------------------------------------
export const adjustInventory = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const { product_id, type, value } = req.body as z.infer<typeof adjustInventorySchema>;
    ...
    const current_stock = product.current_stock as number;
    let quantity_change: number;
    let stock_after: number;
    let reason: 'restock' | 'manual_adjustment';
...
// ---------------------------------------------------------------------------
export const getInventoryLogs = async (req: Request, res: Response): Promise<any> => {
  try {
    const tenant_id = req.user?.tenant_id;
