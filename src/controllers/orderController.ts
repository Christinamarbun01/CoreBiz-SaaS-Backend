...
export type PosOrderPayload = z.infer<typeof posOrderSchema>;
...
// ---------------------------------------------------------------------------
export function failFastQuantityGuard(
  req: Request,
  res: Response,
  next: NextFunction,
): void {
  const items: Array<{ product_id: string; quantity: number }> | undefined = req.body?.items;
...
export async function createPosOrder(
  ...
  try {
    const payload = req.body as PosOrderPayload;
...
export async function payOrder(
  ...
  try {
    ...
    const { method, discount_amount } = req.body as z.infer<typeof payOrderSchema>;
