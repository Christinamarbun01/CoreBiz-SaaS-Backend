...
export async function getPnlReport(req: Request, res: Response, next: NextFunction): Promise<any> {
  try {
    ...
    let startDateStr = req.query.start_date as string;
    let endDateStr = req.query.end_date as string;
    ...
    const dailyData: Record<string, { revenue: number; expense: number; cogs: number }> = {};
    ...
    for (const order of orders) {
      ...
      if (!dailyData[day]) dailyData[day] = { revenue: 0, expense: 0, cogs: 0 };
