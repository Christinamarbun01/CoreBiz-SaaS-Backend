...
// Test DB connection
apiV1Router.get('/test-db', async (_req: Request, res: Response) => {
  try {
...
// Webhook WhatsApp
apiV1Router.post('/webhook/whatsapp', webhookLimiter, (req: Request, res: Response) => {
  const token = (req.headers['x-webhook-token'] as string | undefined) ?? (req.query['token'] as string | undefined);
