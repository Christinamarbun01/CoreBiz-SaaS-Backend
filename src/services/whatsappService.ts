 ...
 */
const PROFILE_DICT: readonly string[] = ['email', 'plat nomor', 'nama'] as const;
 ...
 */
function parsePhoneNumber(jid: string): string {
  return jid.split('@')[0];
}
 ...
 */
function shouldIgnore(jid: string, fromMe: boolean): boolean {
  if (fromMe) return true;                 // Pesan dari nomor sendiri
  if (isJidBroadcast(jid)) return true;   // Broadcast list
...
interface ParsedBlindData {
  /** Key yang cocok dengan PROFILE_DICT → masuk ke profile pelanggan */
  profile_metadata: Record<string, string>;
  /** Key yang tidak cocok → masuk ke custom_data order */
  order_custom_data: Record<string, string>;
}
 ...
 */
function blindParser(text: string): ParsedBlindData {
  const result: ParsedBlindData = { profile_metadata: {}, order_custom_data: {} };

  const lines = text.split('\n').map(l => l.trim()).filter(Boolean);

  for (const line of lines) {
    const separatorIdx = line.indexOf(':');
    if (separatorIdx === -1) continue; // Bukan format key: value — lewati
...
async function handleIncomingMessages(
  ...
  for (const msg of messages) {
    ...
    if (shouldIgnore(jid, fromMe)) continue;

    // Hanya proses pesan teks biasa
    const textContent =
      msg.message?.conversation ??            // Teks langsung
      msg.message?.extendedTextMessage?.text; // Teks dengan preview link

    if (!textContent) continue; // Bukan teks biasa — lewati
    ...
    processWhatsAppOrder(msgId, sender, textContent).catch((err) =>
      console.error('[WhatsApp] processWhatsAppOrder error:', err),
    );
  }
}
...
export async function initializeWhatsApp(): Promise<WASocket> {
  ...
  sock.ev.on('messages.upsert', ({ messages, type }) => {
    ...
    handleIncomingMessages(messages).catch((err) =>
      console.error('[WhatsApp] handleIncomingMessages error:', err),
    );
  });
...
export function getWhatsAppSocket(): WASocket {
  if (!sock) {
    throw new Error(
      '[WhatsApp] Socket belum diinisialisasi. Panggil initializeWhatsApp() terlebih dahulu.',
    );
  }
  return sock;
}
